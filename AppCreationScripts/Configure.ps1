...
#> 
   ...
   Write-Host "Creating the AAD appplication (TodoListService-StoreApp)"
   $serviceAadApplication = New-AzureADApplication -DisplayName "TodoListService-StoreApp" `
                                                   -HomePage "https://localhost:44321/" `
                                                   -IdentifierUris "https://$tenantName/TodoListService-StoreApp" `
                                                   -PublicClient $False
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>TodoListService-StoreApp</a></td></tr>" -Path createdApps.html

   # Create the client AAD application
   Write-Host "Creating the AAD appplication (TodoListClient-StoreApp)"
   $clientAadApplication = New-AzureADApplication -DisplayName "TodoListClient-StoreApp" `
                                                  -ReplyUrls "https://TodoListClient-StoreApp" `
                                                  -PublicClient $True
   ...
   Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>TodoListClient-StoreApp</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]
   # Add Required Resources Access (from 'client' to 'service')
   Write-Host "Getting access from 'client' to 'service'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "TodoListService-StoreApp" `
   ...
   $configFile = $pwd.Path + "\..\TodoListClient\MainPage.xaml.cs"
   Write-Host "Updating the sample code ($configFile)"
   $dictionary = @{ "const string tenant" = $tenantName;"const string clientId" = $clientAadApplication.AppId;"const string todoListResourceId" = $serviceAadApplication.IdentifierUris;"const string todoListBaseAddress" = $serviceAadApplication.HomePage };
